...
class Car extends Factory{
  constructor(model, doors, color, enginetype, transmission, trim, wheelstrim, audio, seatstrim, moonroof, enginesize=4, navigation=true, backupcamera=true, warranty){
    super();
    this.model = model;
    this.doors = doors;
    this.color = color;
...
class Sport extends Car{
  constructor(model, color, transmission, trim, audio, wheelstrim, seatstrim, top, warranty){
    super(warranty);
    this.model = model,
    this.trim = trim,
    this.transmission = transmission,
...
let mazda3 = new Car(model="mazda3", door="2", color="red", enginetype="hybrid", transmission="automatic", trim="touring", wheels="base", audio="premium", seats="leather", moonroof=true);
...
// Write your code below:
// customerBuild(car, color){
//   console.log("Building one " + color +" "+ car.trim + " " + car.model + " with the following options: " + car.weatherpackage );

mazda3.customerBuild(mazda3, "yellow", ['weatherpackage', 'satelliteradio', 'rearspoiler']);
...
miataRf.customerBuild(miataRf, "black", ['hid headlights', 'sports suspension', 'leather steering wheel', 'heated seats', 'adaptive cruise control']);
